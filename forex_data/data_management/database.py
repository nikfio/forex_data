...
TDENGINE CONNECTOR:
    ...
    supertable  : Ticker_YALL_timeframe
        in order to have a table holding data from beginning to end 
        of availability, not divided in years slices
...
class TDengineConnector(DatabaseConnector):
    ...
    def __attrs_post_init__(self, **kwargs):
        ...
        self._db_uri = f"{self.protocol}://{self.user}:{self.password}@{self.target_url}:{self.port}"
    
    
    def connect(self, mode='sqlalchemy'):
        ...
        if mode == 'sqlalchemy':
            ...
            try:
                ...
                logger.error(f'SQLALCHEMY: taos engine creation error: {e}')
                raise
                ...
                try:
                    ...
                    logger.error(f'SQLALCHEMY: taos connect error: {e}')
                    raise
            ...
            try:
                ...
                conn = taos_connect(host       = self.target_url,
                                    user       = self.user,
                ...
                logger.error(f'TAOS: taos connect error: {e}')
                raise
    ...
    def list_database(self):
        ...
        try:
            ...
            read = conn.execute(text("SHOW DATABASES"))
            
        except Exception as e:
            
            logger.error('Error list databases: {self.target_url}')
            
        else:
            
            # parse read outcome
            db_list = [item[0] for item in read.fetchall()]
    ...
    def dict_stables(self):
        
        stables_dict = {}
        
        conn = self.connect()
        
        db_list = self.list_database()
        
        for db in db_list:
            
            try:
                
                read = conn.execute(text(f'use {db}'))
                read = conn.execute(text(f'show stables'))
                
            except Exception as e:
                
                logger.error(f'Error list supertables for {self.target_url}: {e}')
                
                
            else:
        
               stables_dict[db] = [item[0] for item in read.fetchall()]
    ...
    def _list_tables(self, stable):
        ...
        for key, value in self.dict_stables().items():
            
            if (isinstance(value, list)
                and
                stable in value):
                    
                db_stable = (key,stable)
                
                break
        
        try:
                
            tables = conn.execute(text(f'''SELECT table_name
                                            FROM information_schema.ins_tables
                                            WHERE db_name = '{db_stable[0]}'
                                            AND stable_name = '{db_stable[1]}' 
                                        '''))
            
        except Exception as e:
            
            logger.error(f'Error list tables for {self.target_url}, stable {stable}: {e}')
            
            
        else:
    
            tables_list = [item[0] for item in tables.fetchall()]
    ...
    def _to_tdengine_column_types(self, columns_dict):
        ...
        for key, value in columns_dict.items():
            ...
            match key:
                ...
                case BASE_DATA_COLUMN_NAME.TIMESTAMP:
                    ...
                    tdengine_columns_dict['ts'] = 'TIMESTAMP'
                ...
                case BASE_DATA_COLUMN_NAME.ASK \
                    ...
                    tdengine_columns_dict[key] = 'FLOAT'
                    
                case BASE_DATA_COLUMN_NAME.TRANSACTIONS:
                    
                    tdengine_columns_dict[key] = 'BIGINT UNSIGNED'
        ...
        if not list(tdengine_columns_dict.keys())[0] == 'ts':
            
            o_dict = OrderedDict(tdengine_columns_dict.items())
            o_dict = o_dict.move_to_end('ts',
                                        last=False)
    ...
    # forex-data application dependent methods
    ...
    def get_tickers_list(self):
        ...
        stables_dict = self.dict_stables()
        
        for db in stables_dict.keys():
            
            for stable in stables_dict[db]:
                
                if fullmatch(TICKER_PATTERN, stable):
                    
                    tickers_list.append(stable)
    ...
    def get_ticker_keys(self, ticker, timeframe=None):
        ...
        if stables_dict:
            ...
            for db in stables_dict.keys():
                ...
                if ticker in stables_dict[db]:
                    
                    tables = self._list_tables(ticker)
                        
                    if timeframe:
                            
                        if timeframe.lower() in tables:
                            
                            ticker_keys_list.append(self._db_key(ticker,
                                                                 timeframe))
            
                    else:
                        
                        ticker_keys_list.extend(
                            [self._db_key(ticker, table) 
                             for table in tables]
                        )
    ...
    def get_ticker_keys_dict(self, ticker, timeframe=None):
        ...
        if stables_dict:
            ...
            for db in stables_dict.keys():
                ...
                for stable in stables_dict[db]:
                    ...
                    tables = self._list_tables(stable)
                    
                    if stable == ticker.lower():
                    
                        if timeframe:
                            
                            if timeframe in tables:
                                
                                if not db in ticker_keys_dict.keys():
                                    
                                    ticker_keys_dict[db] = list(self._db_key(ticker,
                                                                         timeframe))
                                    
                                else:
                                    
                                    ticker_keys_dict[db].append(self._db_key(ticker,
                                                                         timeframe))
                                        
                        else:
                            
                            keys = [self._db_key(ticker, table) 
                                    for table in tables]
                            
                            if not db in ticker_keys_dict.keys():
                                
                                ticker_keys_dict[db] = keys
                                
                            else:
                                
                                ticker_keys_dict[db].extend(keys)
                            
        return ticker_keys_dict
    ...
    def get_ticker_years_list(self, ticker, timeframe=TICK_TIMEFRAME):
        ...
        ticker_tables_dict = self.get_ticker_keys_dict(ticker)
        
        if ticker_tables_dict:
            
            for db_val, keys in ticker_tables_dict.items():
                
                for key in keys:
                    
                    items = self._get_items_from_db_key(key)
                    
                    if (
                            items[0] == ticker.lower()
                            and
                            items[1] == timeframe.lower()
                        ):
        ...
        if key_found:
            ...
            try:
                ...
                logger.error(f'Error connecting to database {db}: {e}')
                raise
                
            else:
                
                query = f'''SELECT DISTINCT YEAR(ts) FROM {table}'''
            
                read = read_database(text(query), conn)
                    
                # parse response 
                # to have a list of unique values of years
                # available in the stable selected
                            
        return ticker_years_list
    
    
    def write_data(self, database, key, dataframe, connection_mode='sqlalchemy'):
        
        # from key get stable and table
        stable = key.split('.')[0]
        table  = key.split('.')[1]
        
        # prepare table columns
        # get current stables available
        stable_list = self.dict_stables()[database]
        # have column structure for tdengine
        tdengine_cols_dict = self._to_tdengine_column_types(dict(dataframe.collect_schema()))
        
        tdengine_cols_str = ', '.join([f"{key} {tdengine_cols_dict[key]}" 
                                       for key in tdengine_cols_dict])
        dataframe = dataframe.collect()
        
        # open a connection
        conn = self.connect(mode = connection_mode)
        
        if connection_mode == 'sqlalchemy':
            
            # exec db creation
            try:
                
                db_create = conn.execute(text(f'''create database if not exists 
                                              {database} vgroups 10 precision 'ms'
                                              keep 730'''))
            ...
            if not isinstance(db_create, sqlalchemy_CursorResult):
                
                logger.error('SQLALCHEMY: error create database')
                raise
        ...
        elif connection_mode == 'taos':
            
            
            # exec db creation
            try:
                
                db_create = conn.execute(f'''create database if not exists {database} 
                                             vgroups 10 
                                             precision 'ms'
                                             keep 11000d''')
            ...
            # create stable if necessary
            query_create_stable = f'''CREATE STABLE IF NOT EXISTS {stable}
                                    ({tdengine_cols_str}) 
                                    TAGS (timeframe NCHAR(50))'''
            stable_create = conn.execute(query_create_stable)
                                    
            query_create_table = f'''CREATE TABLE IF NOT EXISTS {table}
                                    USING {stable}
                                    (timeframe) TAGS ({table})'''
            #table_create = conn.execute(query_create_table)
                  
            # execute dataframe INSERT into supertable
            cursor = conn.cursor()
            columns_for_insert = ', '.join(list(tdengine_cols_dict.keys()))
            
            placeholders ='"%s", ' + ", ".join(["%s"] * (len(tdengine_cols_dict)-1))
            insert_sql = f'''INSERT INTO {table}
                            USING {stable} (timeframe) TAGS ('{table}')
                            ({columns_for_insert})
                            VALUES 
                            ({placeholders})'''
            insert_sql_no_placeholders = f'''INSERT INTO {table}
                            USING {stable} (timeframe) TAGS ('{table}')
                            ({columns_for_insert})
                            VALUES ''' 
            values_placeholders_line = f'({placeholders}) '
                
            df_ts = dataframe.with_columns(
                dataframe[BASE_DATA_COLUMN_NAME.TIMESTAMP].dt.strftime("%Y-%m-%d %H:%M:%S%.3f").alias(BASE_DATA_COLUMN_NAME.TIMESTAMP)
            )
            
            df_ts = df_ts.rename( {BASE_DATA_COLUMN_NAME.TIMESTAMP : 'ts'} )
         
            batch_size = int((1048576 // (len(insert_sql_no_placeholders.encode('utf-8')))))
            for i in range(0, len(dataframe), batch_size):
                batch_df = df_ts.slice(i, batch_size)
                batch_sql = insert_sql_no_placeholders
                for row in batch_df.iter_rows():
                    values = values_placeholders_line % row
                    batch_sql += values
                write_result = cursor.execute(batch_sql)    
                conn.commit()
    ...
    def read_data(self,
        ...
        if database in self.list_database():
            ...
            try:
                ...
                db_use = conn.execute(text(f"USE {database}"))

            except Exception as e:
                
                logger.error(f'Connection {self._db_uri} '
                             f'database {database} use failed: {e}')
                
            else:
                
                stable = self._db_key(ticker, timeframe)
                start_str = start.strftime(DATE_FORMAT_SQL)
                end_str   = end.strftime(DATE_FORMAT_SQL)
                query = f'''SELECT * FROM {stable} ORDER BY {BASE_DATA_COLUMN_NAME.TIMESTAMP}
                            WHERE {BASE_DATA_COLUMN_NAME.TIMESTAMP} BETWEEN {start_str} AND {end_str}'''

                try:
                
                    dataframe = read_database(query, conn)
