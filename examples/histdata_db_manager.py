...
def _write_timing_report(phase_times: dict, fetch_times_1: list[float], fetch_times_2: list[float]) -> None:
    lines = []
    lines.append("=" * 64)
...
def main():
    ...
    if not is_empty_dataframe(yeardata1):
        logger.bind(target='profiler').debug(
            f"get_data (1D): ticker {ex_ticker}, "
            f"rows {shape_dataframe(yeardata1)[0]}, "
            f"start {get_dataframe_element(yeardata1, BASE_DATA_COLUMN_NAME.TIMESTAMP, 0)}, "
            f"end {get_dataframe_element(yeardata1, BASE_DATA_COLUMN_NAME.TIMESTAMP, shape_dataframe(yeardata1)[0] - 1)}"
        )
    else:
    ...
    if not is_empty_dataframe(yeardata2):
        logger.bind(target='profiler').debug(
            f"get_data (3D): ticker {ex_ticker}, "
            f"rows {shape_dataframe(yeardata2)[0]}, "
            f"start {get_dataframe_element(yeardata2, BASE_DATA_COLUMN_NAME.TIMESTAMP, 0)}, "
            f"end {get_dataframe_element(yeardata2, BASE_DATA_COLUMN_NAME.TIMESTAMP, shape_dataframe(yeardata2)[0] - 1)}"
        )
    else:
